...
find_steady_state!(sam; t=10, dt=3)
...
function preparestate!(estim::SAMEstim, y::Vector{<:Real})
    ...
    update_sam = sam
    # update sam with y
    update_sam.sys_struct.transforms[1].heading = y[1]
    update_sam.sys_struct.transforms[1].elevation = y[2]
    update_sam.sys_struct.transforms[1].azimuth = y[3]
    SymbolicAWEModels.reposition!(update_sam.sys_struct.transforms, update_sam.sys_struct)
    i = 4
    for winch in update_sam.sys_struct.winches
